class Post < ActiveRecord::Base
  ...
  def strip_link
    ...
    if link.include? 'youtube'
       self.embed_link = link.scan(/watch?v=\/\./).first # this sets embed_link paramater of instance of Post class to the last 10 digits of the link
    elsif link.include? 'vimeo'
       self.embed_link = link.scan(/\/\d{1,}/).first
